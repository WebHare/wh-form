...
, initialize: function(formelement, options)
  {
    ...
    if(!this.form.getAttribute('method') && WHBase.config && WHBase.config.dtapstage != 'production')
...
*/
    this.form.addEvents({"focusin":  this._onFocusGain.bind(this)
                        ,"focusout": this.handleFocusLoss.bind(this)
...
//    if(this.form.contains($wh.getCurrentlyFocusedElement()))
  //    this.keyboard.activate();
    ...
    if(WHBase.config && WHBase.config.dtaptype != 'production')
    {
    ...
    var currentfocus = domfocus.getCurrentlyFocusedElement();
...
, fireInitialEvents:function()
  {
    models.processOnModels(this.form, false, function(model)
    {
...
, handleRPCSubmit:function(issubmit, onsuccess, response)
  {
    ...
    {
      legacybase.executeSubmitInstruction(response.submitinstruction);
      return;
    }
    ...
    if(response.submitted)
    {
      ...
      //FIXME $wh.track('form','submitted',this.form);
      if(onsuccess)
        onsuccess();
    }
...
, initializeModel:function(parentmodel, node)
  {
    ...
    if(modelname)
    {
      modeltype = models.modeldefs[modelname.toLowerCase()];
      if(!modeltype)
    ...
    if(!modeltype && node.get('tag')=='input')
    {
      ...
      modeltype = models.modeldefs["wh.submit"];
      /*
      if (modeltype)
    ...
    if(!modeltype && node.get('tag')=='select')
      modeltype = models.modeldefs["wh.select"];

    if(modeltype)
      return new modeltype(this, node, parentmodel);

    return new models.InputFieldBase(this, node, parentmodel);
  }
...
, createGroup:function(node, type, parentmodel)
  {
    ...
    var model = models.getFieldModel(fieldgroup);
    if(model)
    {
    ...
    {
      case "radio":
        new models.modeldefs["wh.radiogroup"](this,fieldgroup,parentmodel);
        break;

      case "checkbox":
        new models.modeldefs["wh.checkboxgroup"](this,fieldgroup,parentmodel);
        break;
    }
  }
...
, initializeModels:function(basenode, parentmodel)
  {
    models.processOnFields(basenode, false, function(field)
    {
      this.initializeModel(parentmodel, field);
    }.bind(this));

    models.processOnModels(basenode, false, function(model)
    {
...
, resetFieldStatusOfHiddenFields: function()
  {
    ...
    var fields=[];
    models.processOnFields(this.form, false, function(node)
    {
      fields.push(node);
    })
...
, setValidationStatus:function(field, result)
  {
    var model = models.getFieldModel(field);
    if(!model)
...
, isPartDirty:function(part)
  {
    return !!models.processOnModels(part, true, function(model) { return model.dirty });
  }
, scanDirtyState:function()
  {
    var dirtylist = [];

    models.processOnModels(this.form, !this.options.debugdirty, function(model) //loop through all only when debugging
    {
      if(model.dirty)
      {
...
, focusForm:function()
  {
    var focusable = domfocus.getFocusableComponents(this.form, false).pick();
    if(focusable)
...
, getValidity:function(basenode)
  {
    ...
    models.processOnModels(basenode, true, function(model)
    {
      ...
      if(!firstinvalid && nonvalidfield.isValid()===false)
      {
        ...
        if(!this.notinteractive)
          0;//FIXME $wh.track('form','invalid', firstinvalid.node);
      }
...
, handleLastFocused:function(focusgainedby)
  {
    ...
    var lastmodel = models.getFieldModel(field);
    ...
    {
      var lastgroupmodel = models.getFieldModel(lastgroup);
      if(lastgroupmodel)
        this.validateField(lastgroupmodel, true, false, true);
      else models.processOnModels(lastgroup, false, function(model)
      {
...
, _onFocusGain:function(event)
  {
    this._handleFocusGain(event.target || moo.$(event.event.srcElement));
  }
...
//    this.keyboard.activate();
    ...
    this.lastfocusedmodel = focusgainedby ? models.getFieldModel(focusgainedby) : null;
    ...
    if(focusgainedby && !this.formstarted && !this.notinteractive) //form starting
    {
      ...
      //FIXME $wh.track('form','start',this.form);
    }
    ...
    if(this.lastfocused)
    {
      if(domfocus.getFocusableElement(focusgainedby) == domfocus.getFocusableElement(this.lastfocused)) //same element returned focus, ignore
        return;
...
, handleFocusLoss:function(event)
  {
    ...
    var focuslostby = event.target || moo.$(event.event.srcElement);
    if(focuslostby)
...
, focusFieldError:function(failedmodel)
  {
    var evt = new FocusEvent(this, failedmodel, failedmodel.node);
...
, submitPart:function(tosubmit, usesubmitbutton, onsubmitted)
  {
    ...
    dosubmit = domevents.fireHTMLEvent(this.form, 'submit');
...
, resetPart:function(part)
  {
    ...
    models.processOnModels(part, false, function(model)
    {
      model.reset();
    });
...
, getStateForApi:function(tosubmit, issubmit) //returns a value suitable for the formsapi
  {
    ...
    if(issubmit)
    {
      var seenvars={};
      retval.apivars=[];
      models.processOnModels(tosubmit, false, function(model)
      {
        var value = model.getApiValue();
...
, applyApiResultFields:function(fields, options)
  {
    this.setBlockDirtyEvents(true);
    models.processOnModels(this.form, false, function(model) //always walk in local order, not server order
    {
...
, handleKeyDown:function(event)
  {
    ...
    var model = models.getFieldModel(event.target);
    if(model)
      model.setDirty(true);
  }
...
, updateEnableOnState:function(enableonchanges, node)
  {
    ...
    var model = models.getFieldModel(node);
    if(model)
      model.setEnabled(shouldshow);

    models.processOnModels(node, false, function(model)
      {
...
/** @short
    ...
    @options
*/
function setupHandler(item, options)
{
  if(!options)
    options={};
  ...
  if(form)
    ...
    form = new Handler(item, options);
  return form;
}

function setupHandlers(selector, options)
{
  /*
  ...
  */
  ...
  moo.$$(selector).each(function(item)
  {
    setupHandler(item, options);
  });
}

var GroupableInputsBase = new Class(
{ Extends: models.InputGroupBase
, getName:function()
  {
...
, setRawValueByArray:function(values)
  {
    Array.each(this.getGroupMembers(), function(member)
    {
      ...
      if(member.checked != setvalue)
      {
        ...
        domevents.fireHTMLEvent(member, "input");
        domevents.fireHTMLEvent(member, "change");
      }
    });
  }
...
models.modeldefs["wh.radiogroup"] = new Class(
...
models.modeldefs["wh.checkboxgroup"] = new Class(
{ Extends: GroupableInputsBase
, minselected: 0
, maxselected: 0
...
models.modeldefs["wh.select"] = new Class(
{ Extends: models.InputFieldBase
, initialize: function(formhandler, node, parentmodel)
  {
    this.parent(formhandler, node, parentmodel);
  }
...
models.modeldefs["wh.submit"] = new Class(
{ Extends: models.InputFieldBase
, getValue:function()
  {
