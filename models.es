...
{
  var model = getFieldModel(field);
  if(model)
    return model.getValue();

  console.error("getValue: The requested field has no model",field);
}
function setValue(field, value)
{
  ...
  console.error("setValue: The requested field has no model",field);
}
...
function processOnFields(parents, stopfirstsuccess, callback)
{
  ...
  for(var i=0;i<fullparents.length;++i)
  {
    ...
    var seennames = [];
    var retval = processOnFieldsRecursive(findbasename, fullparents[i], fullparents[i], stopfirstsuccess, callback, seennames);
    if(retval && stopfirstsuccess)
      return retval;
  }
}
function processOnFieldsRecursive(findbasename, origparent, currentparent, stopfirstsuccess, callback, seennames)
{
  for(var node = currentparent.firstChild; node; node = node.nextSibling)
  {
    try
    {
      if(!legacybase.isHTMLElement(node) || $(node).hasClass("templaterow") || node.name == 'formsapi-retentionid' || node.retrieve("wh-form-parentmodel"))
        continue;
    }
    catch(e)
    {
...
//          continue;
    ...
    var retval = processOnFieldsRecursive(findbasename, origparent, node, stopfirstsuccess, callback, seennames);
    if(retval && stopfirstsuccess)
      return retval;
  }
}
function processOnModels(parent, stopfirstsuccess, callback)
{
  return processOnFields(parent, stopfirstsuccess, function(field, localname)
  {
    var model = getFieldModel(field);
    if(!model)
      return;
...
function setValidator(field, validator)
{
  if (field == null)
...
function setEnableOnCheck(field, enableonchecker)
{
  if(field instanceof Elements || field instanceof Array)
  {
    Array.each(field, function(el) { setEnableOnCheck(el, enableonchecker )});
    return;
  }
  field=$(field);
...
//    console.error("aplplystatus lookup", this, relevantstatus);
    ...
    {
  //    console.log('test ', searchgroup, getFieldModel(searchgroup));
      if(!legacybase.isHTMLElement(searchgroup) || searchgroup == this.formhandler.form || getFieldModel(searchgroup)) //found a different field/model
        break; //give up
...
, setFieldErrorMessage:function(statusnode, errormsg)
  {
    ...
    if(!errorholder) //we'll need to create one ?
    {
      ...
      {
        errorholder.inject(domfocus.getFocusableElement(this.node), this.formhandler.options.errorfieldplacement);
      }
    ...
    else if(errormsg === null) //clean existing error holder
    {
      ...
      {
        legacybase.setTextWithLinefeeds(errorholder,'');
      }
      return;
    }
    legacybase.setTextWithLinefeeds(errorholder,errormsg);
    return;
  }
...
, fireChangeEvents:function()
  {
    ...
    {
      var evt = new domevents.CustomEvent("change", {bubbles:true, cancelable:true});
      domevents.dispatchEvent(this.node, evt);
    }
  }
});

let InputFieldBase = new Class(
{ Extends: ModelBase
, needschecked: false
, filename:null
, filevalue:null
...
, fireChangeEvents:function()
  {
    this.parent();
    domevents.fireHTMLEvent(this.node, "input");
  }
});

let InputGroupBase = new Class(
{ Extends: ModelBase
, initialize: function(formhandler, node, parentmodel)
  {
    this.parent(formhandler, node, parentmodel);
...
//    var basename = this.node.hasAttribute('data-form-flatten') ? '' : this.getName() + this.__fieldseparator;
    ...
    processOnModels(this.node, false, function(model, localname)
    {
...
, updateStatus: function()
  {
    ...
    processOnModels(this.node, false, function(submodel)
    {
...
//    if(!this.node.hasAttribute("data-form-flatten"))
  //    return null;
    ...
    var submodels = [];
    processOnModels(this.node, false, function(model) { submodels.push(model); }, false); //grab the models without recurinsg into getProcessableSubModels
    return submodels;
  }
, setAllowFeedback:function(allowfeedback)
  {
...
, initialize:function(form, node, parentmodel)
  {
    ...
    Array.each(rows,function(row)
    {
      ...
      var rowmodel = new modeldefs["wh.record"](this.formhandler, row.node);
...
, addRow:function(template)
  {
    ...
    this.setDirty(true);
    domui.applyReplaceableComponents(newrow);
    legacybase.fireLayoutChangeEvent(this.node);
    //this.formhandler.initializeModels(newrow);

    var rowmodel = new modeldefs["wh.record"](this.formhandler, newrow, this);
...
, initialize:function(form, node, parentmodel)
  {
    ...
    Array.each(this.node.getElements(".editbutton[data-editform]"), function(editbutton)
    {
      ...
      processOnFields(formnode, false, function(field)
      {
...
, onEditButton:function(event, target)
  {
    ...
    var editform = getHandler(editformname);
    if(!editform)
...
, getModelFor:function(name)
  {
    ...
    {
      var form = getHandler(editbutton.getAttribute("data-editform"));
      if(form)
...
, getProcessableSubModels:function()
  {
   ...
   //grab all submodels in forms
    Array.each(this.node.getElements(".editbutton[data-editform]"), function(editbutton)
    {
      ...
      processOnModels(form.form, false, function(model)
      {
...
modeldefs["wh.record"] = RecordBase;
modeldefs["wh.union"] = UnionBase;
modeldefs["wh.recordarray"] = RecordArrayBase;
modeldefs["wh.hiddenarray"] = HiddenArrayBase;
...
module.exports = { getHandler: getHandler
                 ...
                 , getValue: getValue
                 , setValue: setValue
                 , processOnFields: processOnFields
                 , processOnFieldsRecursive: processOnFieldsRecursive
                 , processOnModels: processOnModels
                 , setValidator: setValidator
                 , setEnableOnCheck: setEnableOnCheck
                 , ModelBase: ModelBase
                 , InputGroupBase: InputGroupBase
